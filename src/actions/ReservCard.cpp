...
///Reste a faire sur ce fichier : Questionnements condition et BaseCardUpdate cf fichier .cpp
...
void Splendor::ReservCard:: BaseCardUpdate(Board& b, Player& p) const {
    ...
    ///Q: est ce que cette fonction a bien été implémentée?
    ///Modifie t'elle le plateau en ajoutant une nouvelle carte de la pioche au plateau?
}

bool Splendor::ReservCard::condition(Board &b, Player &p) const {
    //Chaque joueur peu avoir au max 3 cartes reservées (fait)
    /// Q : faut-il verifier que la carte entrée en attribut est bien presente sur le plateau ? (pas fait) ///
