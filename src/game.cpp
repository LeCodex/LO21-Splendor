...
void Splendor::Game::playAI(Splendor::Player &p, int level)
{
    ...
    if (level == 1)
    {
        ...
        for (int i = 0; i < 3; i++)
        {
            if (p.getReservedCard(i))
                if (buyReservedCard(*p.getReservedCard(i), p))
                    return; //L'IA a acheté sa iéme carte reservée
        }

        ///2 )Recherche de la premiere carte qu'elle peut acheter sur le plateau et achat eventuel
        for (int i = 2; i >= 0; i--)
        { //l'IA essaye d'abord d'acheter les cartes du haut du plateau car il y a plus de points de prestige generalement.
            for (int j = 3; j >= 0; j--)
            {
                if (board.getCard(i, j))
                    if (buyBoardCard(*board.getCard(i, j), p))
                        return; //L'IA a acheté la carte du plateau d'indice i,j
            }
        }
        ...
        { //Action retenue : Pioche de 3 token diffèrents
            for (int t1 = White; t1 != Gold; t1++)
            {
                for (int t2 = Blue; t2 != Gold; t2++)
                {
                    ...
                    {
                        if (takeThreeDifferentToken((Token)t1, (Token)t2, (Token)t3, p))
                            return; // l'IA a pris ses trois jetons
                    }
                }
            }
            ...
            for (int t = White; t != Gold; t++)
            {
                if (takeTwoIdenticalToken((Token)t, p))
                    return; // l'IA a pris ses 2 jetons
            }
        }
        else
        { //Action retenue :Pioche de 2 token identiques
            for (int t = White; t != Gold; t++)
            {
                if (takeTwoIdenticalToken((Token)t, p))
                    return; // l'IA a pris ses 2 jetons
        ...
        for (int i = 0; i != 3; i++)
        {
            if (reserveDrawCard(i, p))
                return; // L'IA a reservé une carte de la pioche i
        }
    ...
    if (level == 0)
    {
        ...
        if (reserveCenterCard(*board.getCard(0, 0), p))
            return; //reservation de la premier carte du plateau
